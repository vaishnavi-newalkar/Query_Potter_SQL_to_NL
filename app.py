...
def main():
    ...
    # ---------------------------------
    session = Session(faiss=faiss_idx,db_path=db_path, schema=cache.get(), fk_graph=fk_graph)
    llm = GroqClient()
    ...
    while True:
        ...
        # Schema refinement
        # ------------------------------
        refined_schema = refine_schema(session.faiss,session.full_schema, user_query,fk_graph)
