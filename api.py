...
from schema.refiner import refine_schema , build_faiss_index
...
async def upload_db(file: UploadFile = File(...)):
    ...
    cache = SchemaCache()
    cache.load(schema)
    faiss_idx, _,fk_graph= build_faiss_index(schema)
    session = Session(faiss_idx,db_path=db_path, schema=cache.get(),fk_graph=fk_graph)
...
def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        if is_meta:
            ...
            session.memory.add_turn(user_input, result["answer"])
        ...
        refined_schema = refine_schema(session.faiss,session.full_schema, user_query,session.fk_graph)
        ...
        try:
            gen_res = generate_sql_with_reasoning(llm, plan, refined_schema, user_query,history=history)
            sql = gen_res["sql"]
        ...
        session.memory.add_turn(user_input,final_answer)
...
def get_session_info(session_id: str):
    ...
    return {"session_id": session_id, "tables": list(session.full_schema.keys()), "chat_history_length": len(session.memory.history.messages), "has_pending_clarification": session.clarification_state is not None}
